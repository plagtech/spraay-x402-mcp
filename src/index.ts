...
function registerTools(server: McpServer, api: any) {
  ...
  // ============================================
  // AI INFERENCE (4 tools) — NEW in v3.0.0
  // ============================================

  server.tool(
    "spraay_inference_classify_address",
    "AI-powered wallet classification with risk scoring. Analyzes on-chain data to classify addresses as whale, retail, MEV bot, contract, exchange, etc. Returns risk level, confidence score, and behavioral patterns. Costs $0.008 USDC.",
    {
      address: z.string().describe("Wallet address to classify (e.g. 0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045)"),
    },
    async ({ address }) => {
  ...
  server.tool(
    "spraay_inference_classify_tx",
    "AI-powered transaction classification with risk scoring. Decodes and analyzes any Base transaction — identifies type (swap, transfer, mint, bridge, etc), involved protocols, value flows, and risk indicators. Costs $0.008 USDC.",
    {
      hash: z.string().describe("Transaction hash to classify (e.g. 0xabc123...)"),
    },
    async ({ hash }) => {
  ...
  server.tool(
    "spraay_inference_explain_contract",
    "AI-powered smart contract analysis. Fetches verified source code from Basescan, explains what the contract does, lists key functions, identifies security properties, and assesses risk. Works best with verified contracts. Costs $0.01 USDC.",
    {
      address: z.string().describe("Contract address on Base (e.g. 0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913 for USDC)"),
    },
    async ({ address }) => {
  ...
  server.tool(
    "spraay_inference_summarize",
    "AI intelligence briefing for any address or transaction on Base. Returns a structured risk assessment with headline summary, key findings, risk scoring, and actionable recommendations. Costs $0.008 USDC.",
    {
      target: z.string().describe("Address or transaction hash to analyze"),
      context: z.string().optional().describe("Analysis context: 'defi', 'security', 'compliance', 'general' (default: general)"),
    },
